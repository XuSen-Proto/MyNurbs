...
%    end   
   selidx = max(0,i-t):mpi;
   bezalfs(selidx+1,i+1) = (inv*bincoeff(d,selidx).*bincoeff(t,i-selidx))';
end                                                       %   } 
                                                          % 
...
%    end 
   selidx = max(0,i-t):mpi;
   bezalfs(selidx+1,i+1) = bezalfs(d+1-selidx,ph-i+1);
end                                                       %   } 
                                                          % 
mh = ph;                                                  %   mh = ph;       
kind = ph+1;                                              %   kind = ph+1; 
...
%             end
            tmp1 = alfs(q-s+1);
            bpts(:,q+1) = bpts(:,q:q+1)*[1-tmp1 tmp1]';
         end                                              % 
...
%          end                                                  
%       end
      selidx = max(0,i-t)+1:mpi+1;
      ebpts(:,i+1) = bpts(:,selidx)*bezalfs(selidx,i+1);
   end                                                    %     } 
...
%                   end                                     %             }
                  ebpts(:,kj+1) = ebpts(:,kj+1:kj+2)*[gam 1-gam]';
               else                                       %             else  { 
%                   for ii=0:mc-1                           %               for (ii = 0; ii < mc; ii++) 
...
%                   end   
                  ebpts(:,kj+1) = ebpts(:,kj+1:kj+2)*[bet 1-bet]';
               end                                        %             } 
            end                                           %           } 
            i = i + 1;                                    %           i++; 
            j = j - 1;                                    %           j--; 
...
%          end  
         ik(kind+1:kind+1+ph) = ub*ones(1,ph+1);
      end                                                     
end                                                       %   } 
